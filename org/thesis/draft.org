...
interpret: ~data Op = BEGIN_ROUND | END_ROUND | MATCH Match | PERFORM_SORTING
Focus SortMethod~. The key detail is that an interpreter must not proceed past
an ~END_ROUND~ or a ~PERFORM_SORTING~ until all the relevant matches have been
completed[fn::For ~END_ROUND~, all matches are relevant. For ~PERFORM_SORTING~,
only those matches under the ~Focus~ specified are.].
...
* COMMENT A Library for Tournament Design
** Goals
1. Easy to use
...
* COMMENT Availability of this library
